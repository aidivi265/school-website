...
export default function DownloadsClient({ initialDocuments }: { initialDocuments: DocumentItem[] }) {
  const { documents: liveDocuments } = useDocuments(initialDocuments);

  const publishedDocs = liveDocuments.filter((d) => d.is_published !== false);
  ...
  return (
    <div className="space-y-12">
      {categories.map((cat) => {
        const catDocs = publishedDocs.filter((d) => d.category === cat);
